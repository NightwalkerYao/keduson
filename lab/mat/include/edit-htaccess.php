...
if(isset($_POST['pwd'])){
    ...
    if(password_verify($pwd, $correct)){
        ...
        $form = false;
        ?>
            <h2> Editer le fichier .htaccess principal</h2>
            <br>
            <form method="post" id="uphtaccess">
            <center>
                <?php $content = ''; $content .= file_get_contents('../.htaccess'); ?>
                <textarea name="content" rows="18" cols="85" speelcheck="false"><?=$content;?></textarea>
                <br>
                <br>
            ...
            <script>
                $(document).ready(function(){
                    $('#uphtaccess').on('submit', function(e){
                        e.preventDefault();
                        var data = $(this).serialize(), url= 'ajax/edit-htacs.php';
