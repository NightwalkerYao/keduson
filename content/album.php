...
<div id="content">
	<div class="leftC">
				...
				<div>
					<h1>
						<a href="<?=ABOUT_ARTIST.'/'.urlencode(html_entity_decode($T['artiste']));?>">
							<img src="<?=ROOT_SANS;?>/css/images/google.png"> Détails de l'album :: <?=$T['artiste'].' - '.$T['nom'].' ('.$T['annee'];?>)
						</a>
					</h1>
				</div>
				<div class="clear"></div>
		...
		<div class="filmborder">
			<div class="filmcontent">
				<div class="yazitip"> <?=$T['artiste'].' - '.$T['nom'];?> </div>
				<div class="filmalti">
					<div class="filmaltiimg">
						<a href="<?=ABOUT_ALBUM.'/'.$T['code_name'];?>">
							<img style="padding: 0 0px 0 0; width: 390px; height: 400px;" src="<?=ROOT_SANS.'/covers/'.$T['pochette'];?>" alt="<?=$T['artiste'].' - '.$T['nom'];?>" />
						</a>
					</div>
					<div class="filmaltiaciklama" style="margin-left: 24em; width: 300px;">
						...
						<p>
							<span>Genre</span>: 
							<a href="<?=SEARCH_BASE.'?q='.urlencode($T['genre']);?>&amp;content=search&amp;o=date#searchalbums" rel="category tag"><?=$T['genre'];?></a>
						</p>
						<p>
							<span>Année</span>: 
							<?=$T['annee'];?>
						...
						<p>
							<span style="width: auto;">Nombre de pistes</span>: 
							<?=count(explode(';', $T['id_titres']));?>
						</p>
						<p>
							<span style="width: auto;">Vues</span>: 
							<?=$T['hits'];?> fois
						</p>
						<p>
		...
		<div class="filmborder">
			<div class="filmcontent">
				<div class="yazitip">Tracklist ::  <?=$T['artiste'].' - '. $T['nom'];?>
...
<?php
$a = 0;
$tids = explode(';', $T['id_titres']);
foreach ($tids as $t) {
	$a++;
	$same_artist = $connect_bdd->prepare("SELECT * FROM musics WHERE id=?");
	$same_artist -> execute(array(intval($t)));
	$sa = $same_artist->fetch();
		...
		<div class="sidebarborder">
			<div class="sidebar-right">
				<div class="featlist">
					...
					<ul>
<?php
$sugs = $connect_bdd->query("SELECT * FROM albums WHERE moderation=0 AND id >= RAND()*( SELECT MAX(id) FROM albums ) ORDER BY id LIMIT 15");
while($sug = $sugs -> fetch()){
	echo '<li>';
	echo '<a href="'.ABOUT_ALBUM.'/'.$sug['code_name'].'">'.$sug['artiste'].' - '. $sug['nom'].'</a>';
	echo '</li>';
}
$sugs->closeCursor();
?>
